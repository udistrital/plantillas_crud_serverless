...
COLLECTION = "plantilla_vc_plantilla"
...
class PlantillaModel(BaseModel):
    """Modelo de datos de Plantilla (v2)"""
...
def create(data, collection):
    try:
        print("[Plantilla v2] Create: ", data)
...
def update(_id, data, collection):
    try:
        ...
        print(f"[Plantilla v2] Error service Put: {ex}")
        return format_response({}, f"Error service Put: {ex}", 500, False)


def delete(_id, data, collection):
    """Borrado lógico: preserva la integridad referencial con plantilla_version"""
    try:
        print("[Plantilla v2] Delete: ", _id)
        filter_ = {"_id": ObjectId(_id)}
        result = collection.update_one(filter_, {"$set": data})
        if result.matched_count:
            updated_data = collection.find_one(filter_)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if http_method == 'POST':
            ...
            if error is None:
                # Validate structure
                plantilla_data, error = parse_model(PlantillaCreationModel, data)
                if error is not None:
                    return format_response({}, f"Error registering new plantilla! Detail: {error}", 400, False)
                ...
                return format_response({}, "Error registering new plantilla! Detail: Error in input data", 400, False)
            ...
            if error is None:
                ...
                plantilla_data, error = parse_model(PlantillaUpdateModel, data)
                if error is not None:
                    return format_response({}, f"Error updating plantilla! Detail: {error}", 400, False)
                client = connect_db_client()
                if client:
                    plantilla_collection = client[str(PLANTILLAS_CRUD_DB)][COLLECTION]
                ...
                return format_response({}, "Error updating plantilla! Detail: Error in input data", 400, False)
